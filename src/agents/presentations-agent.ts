...
} from "./single-presentation-message-schema";
import { z } from "zod";
...
export class Presentations extends Agent<Env, PresentationAgentState> {
	...
	updateSlideContent(slideId: string, content: Slide) {
	...
	async handleMessage(connection: Connection<unknown>, message: any) {
		...
		try {
			...
			if (parsedMessage.type === "consolidate-messages") {
				...
				console.log("getting agent with id", parsedMessage.slideId);
				const agent = await getAgentByName(
					SinglePresentationAgent,
					parsedMessage.slideId,
				);
				console.log("AGENT obtained", agent);
				const allMessages = await agent.getMessages();
				...
				console.log("existing slide found, content:", slideContent);
				let slideSchema:
					| typeof FullSizeImageSlideSchema
					| typeof OneTextColumnSlideSchema
					| typeof TwoTextColumnsSlideSchema
					| typeof TwoColumnsWithImageSlideSchema
					| typeof TitleSlideSchema
					| typeof BigImageWithCaptionSlideSchema
					| typeof BackgroundImageWithTextSlideSchema =
					FullSizeImageSlideSchema;
				if (slideContent?.design === "title") {
					slideSchema = TitleSlideSchema;
				} else if (slideContent?.design === "full-size-image") {
					slideSchema = FullSizeImageSlideSchema;
				} else if (slideContent?.design === "one-text-column") {
					slideSchema = OneTextColumnSlideSchema;
				} else if (slideContent?.design === "two-text-columns") {
					slideSchema = TwoTextColumnsSlideSchema;
				} else if (slideContent?.design === "two-columns-with-image") {
					slideSchema = TwoColumnsWithImageSlideSchema;
				} else if (slideContent?.design === "big-image-with-caption") {
					slideSchema = BigImageWithCaptionSlideSchema;
				} else if (slideContent?.design === "background-image-with-text") {
					slideSchema = BackgroundImageWithTextSlideSchema;
				}
				console.log("Defined slide schema", slideSchema);
				try {
  ...
  - do-not-update-slide: When you do not need to update the slide
    Parameters:
      - slideId: string (ID of the slide to update)
...
${parsedMessage.message}
  ...
  `,
						tools: {
							"do-not-update-slide": tool({
								description: "Do not update the slide",
								parameters: z.object({
									slideId: z.string().describe("The ID of the slide to update"),
								}),
								execute: async ({ slideId }) => {
									console.log("Do not update slide", slideId);
								},
							}),
							"update-slide": tool({
								...
								execute: async ({ slideId, content }) => {
									console.log("Updating slide", slideId, content);
